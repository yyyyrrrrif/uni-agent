#!/usr/bin/env bash
# ci_test.sh — full CI test suite for agent_aware_router.
#
...
echo "=== agent_aware_router CI tests ==="
STAGES=""
...
if $RUN_E2E; then
    ...
    cleanup_processes
    # VLLM_PORT makes colocated replicas rendezvous on the same fixed port
    # (EADDRINUSE); proxies hijack runner -> gateway calls (ConnectError).
    unset VLLM_PORT HTTP_PROXY HTTPS_PROXY http_proxy https_proxy
    set +e

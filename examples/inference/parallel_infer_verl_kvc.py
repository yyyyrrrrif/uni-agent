...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum response length (tokens). Overrides the task config's max_total_tokens when set; "
        "if omitted, response_length derives from max_total_tokens for parity with parallel_infer_verl.py.",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
